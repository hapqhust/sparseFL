...
class BasicServer():
    def __init__(self, option, model, clients, test_data=None):
        ...
        self.clients_per_round = max(
            int(self.num_clients * option['proportion']), 1)
    ...
    def run(self):
        ...
        filepath = os.path.join(
            self.log_folder, self.option['task'], self.option['dataidx_filename']).split('.')[0]
        if not Path(filepath).exists():
            os.system(f"mkdir -p {filepath}")
        logger.save(os.path.join(
            filepath, flw.output_filename(self.option, self)))
    ...
    def iterate(self, t):
        ...
        self.model = self.aggregate(models, p=[
                                    1.0 * self.client_vols[cid]/self.data_vol for cid in self.selected_clients])
    ...
    def sample(self):
        ...
        while(len(active_clients) < 1):
            active_clients = [cid for cid in range(
                self.num_clients) if self.clients[cid].is_active()]
        # sample clients
        ...
        if self.sample_option == 'uniform':
            ...
            selected_clients = list(np.random.choice(
                all_clients, self.clients_per_round, replace=False))
    ...
    def aggregate(self, models, p=[]):
        ...
        if self.agg_option == 'weighted_scale':
            ...
            w = fmodule._model_sum(
                [model_k * pk for model_k, pk in zip(models, p)])
...
class BasicClient():
    def __init__(self, option, name='', train_data=None, valid_data=None):
        ...
        self.batch_size = len(
            self.train_data) if option['batch_size'] == -1 else option['batch_size']
        ...
        self.drop_rate = 0 if option['net_drop'] < 0.01 else np.random.beta(
            option['net_drop'], 1, 1).item()
        self.active_rate = 1 if option['net_active'] > 99998 else np.random.beta(
            option['net_active'], 1, 1).item()
        self.wandb = option['wandb']
        self.server_gpu_id = option['server_gpu_id']
    ...
    def train(self, model):
        ...
        """
        model.train()
        device = f"cuda:{self.server_gpu_id}" if torch.cuda.is_available(
        ) else 'cpu'
